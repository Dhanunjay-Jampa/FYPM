...
+                <Badge variant="primary">{myTeam.domain?.replace('-', ' ')}</Badge>
               </div>
               <div>
...
+                    {(log.challenges || []).map((challenge: string, index: number) => (
...
+        {weeklyLogs.length === 0 && (
+          <div className="text-center py-12">
+            <h3 className="text-lg font-medium text-gray-900 mb-2">No Weekly Logs</h3>
+            <p className="text-gray-600 mb-4">Submit your first weekly log to track progress.</p>
+            <Button onClick={() => setShowLogForm(true)}>
+              <Plus className="w-4 h-4 mr-2" />
+              Submit First Log
+            </Button>
...
+            <p className="text-gray-600 mb-4">Upload your first document to share with your team.</p>
+            <Button onClick={() => setShowDocumentUpload(true)}>
+              <Upload className="w-4 h-4 mr-2" />
